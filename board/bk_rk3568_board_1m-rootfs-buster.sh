...
# user define, adapt to custom content

# check board name
[ "$BOARD_NAME" != "rk3568_board_1m" ] &&
	error "board name($BOARD_NAME) is not rk3568_board_1m, please check" &&
	return 1

# Directory contains the target rootfs
TARGET_ROOTFS_DIR="binary"

[ -e "${PRDIR}/$TARGET_ROOTFS_DIR" ] &&
	sudo rm -rf "${PRDIR}/$TARGET_ROOTFS_DIR"
...
[ ! -e linaro-buster-alip-*.tar.gz ] &&
	warn "Run mk-base-debian.sh first"

finish() {
	sudo umount "${PRDIR}/"$TARGET_ROOTFS_DIR/dev
	exit -1
}
trap finish ERR
...
# packages folder
sudo mkdir -p "${PRDIR}/"$TARGET_ROOTFS_DIR/packages
sudo cp -rfv "${PRDIR}/"packages/$ARCH/* "${PRDIR}/"$TARGET_ROOTFS_DIR/packages

# overlay folder
sudo cp -rf "${PRDIR}/$BOARD_NAME/"overlay/* $TARGET_ROOTFS_DIR/

# overlay-firmware folder
sudo cp -rf "${PRDIR}/$BOARD_NAME/"overlay-firmware/* $TARGET_ROOTFS_DIR/

if [ "$VERSION" == "debug" ]; then
	# overlay-debug folder
	# adb, video, camera test file
	sudo cp -rf "${PRDIR}/$BOARD_NAME/"overlay-debug/* $TARGET_ROOTFS_DIR/
fi

## hack the serial
sudo cp -f "${PRDIR}/$BOARD_NAME/"overlay/usr/lib/systemd/system/serial-getty@.service \
